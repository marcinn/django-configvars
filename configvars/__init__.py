...
class Config:
    def __init__(self, settings_module=None, env_prefix=""):
    ...
    def initialize(self, settings_module=None, env_prefix=None):
        self._initialized = False

        if not settings_module and not self._settings_module:
            base_path = os.getenv("DJANGO_SETTINGS_MODULE").split(".")[:-1]
            base_path.append(DEFAULT_LOCAL_SETTINGS_MODULE_NAME)
            self._settings_module = ".".join(base_path)
        elif settings_module:
            self._settings_module = settings_module

        try:
            self._local = importlib.import_module(self.settings_module)
            ...
            if self._settings_module:
                raise ImproperlyConfigured(
                    f"Can't import local settings module " f"{self.settings_module}"
                ) from exc
            else:
                self._local = object()
                self._import_module_failed = True
        else:
...
def initialize(settings_module=None, env_prefix=None):
    return DEFAULT_CONFIG.initialize(
        settings_module=settings_module, env_prefix=env_prefix
    )
